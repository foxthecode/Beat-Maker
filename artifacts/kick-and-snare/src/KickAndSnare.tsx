...
export default function KickAndSnare(){
  ...
  useEffect(()=>{
    ...
    return()=>{document.removeEventListener('mouseover',onOver);document.removeEventListener('mousemove',onMove);};
  },[]);
  ...
  const [padFxTab,setPadFxTab]=useState<string>('OUT');
  // ── CP-F states ──
    ...
    <div style={{height:"100dvh",background:th.bg,color:th.text,fontFamily:"'JetBrains Mono','SF Mono','Fira Code',monospace",overflow:"hidden",touchAction:"manipulation",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,minHeight:0,overflowY:"auto",overflowX:"hidden"}}>
            ...
            {atO.map((track)=>{
              ...
              <div key={track.id} style={{height:"100%"}}>
                ...
                <div style={{position:"relative",height:"100%"}}>
                  ...
                  >
                    <div style={{width:28,height:22,borderRadius:6,border:`1px solid ${padFxTrack===track.id?"rgba(191,90,242,0.6)":"rgba(191,90,242,0.3)"}`,background:padFxTrack===track.id?"rgba(191,90,242,0.25)":"rgba(191,90,242,0.08)",color:`rgba(191,90,242,${padFxTrack===track.id?"1":"0.7"})`,fontSize:7,fontWeight:800,letterSpacing:"0.04em",display:"flex",alignItems:"center",justifyContent:"center",pointerEvents:"none",flexShrink:0}}>SFX</div>
                  </div>
        ...
        {hintMode&&!isMobileRef.current&&hoverMsg&&(()=>{
          ...
          let lx=mousePos.x+16;let ty=mousePos.y+16;
          if(lx+280>vw)lx=mousePos.x-296;
          if(ty+80>vh)ty=mousePos.y-80;
          return(
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,minWidth:0}}>
            ...
            <div onPointerDown={onPD} style={{flex:1,position:"relative",height:28,cursor:"ew-resize",touchAction:"none",minWidth:0,display:"flex",alignItems:"center"}}>
              <div style={{position:"absolute",left:0,right:0,height:3,borderRadius:2,background:"rgba(255,255,255,0.1)"}}>
                <div style={{height:"100%",width:`${pct*100}%`,borderRadius:2,background:color,opacity:dimmed?0.4:1}}/>
              </div>
              <div style={{position:"absolute",left:`${pct*100}%`,transform:"translateX(-50%)",width:14,height:14,borderRadius:"50%",background:dimmed?color+"55":color,boxShadow:dimmed?"none":`0 0 5px ${color}99`,border:"2px solid rgba(0,0,0,0.6)",pointerEvents:"none"}}/>
            </div>
        ...
        <>
          ...
          <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:1201,background:"#1c1c1e",borderRadius:"16px 16px 0 0",boxShadow:"0 -8px 40px rgba(0,0,0,0.7)",paddingBottom:"env(safe-area-inset-bottom,12px)"}}>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"2px 14px 6px"}}>
              <span style={{fontSize:10,fontWeight:800,color:tr.color,letterSpacing:"0.08em"}}>SFX · {tr.icon} {tr.label}</span>
              ...
              {padFxTab==='OUT'&&(
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:10,minWidth:0}}>
                    ...
                    <div onPointerDown={(e:React.PointerEvent<HTMLDivElement>)=>{
                      e.preventDefault();e.stopPropagation();
                      const el=e.currentTarget;
                      const setFromX=(cx:number)=>{const r=el.getBoundingClientRect();const pct=(cx-r.left)/r.width;updFx({pan:Math.max(-100,Math.min(100,Math.round((pct*200)-100)))});};
                      setFromX(e.clientX);
                      const onMv=(pe:PointerEvent)=>setFromX(pe.clientX);
                      const onUp=()=>{window.removeEventListener("pointermove",onMv);window.removeEventListener("pointerup",onUp);};
                      window.addEventListener("pointermove",onMv);window.addEventListener("pointerup",onUp,{once:true});
                    }} style={{flex:1,position:"relative",height:28,cursor:"ew-resize",touchAction:"none",minWidth:0,display:"flex",alignItems:"center"}}>
                      {/* center tick */}
                      <div style={{position:"absolute",left:"50%",transform:"translateX(-50%)",width:1,height:10,background:"rgba(255,255,255,0.2)",borderRadius:1,pointerEvents:"none"}}/>
                      {/* track */}
                      <div style={{position:"absolute",left:0,right:0,height:3,borderRadius:2,background:"rgba(255,255,255,0.08)"}}>
                        {(()=>{const p=f.pan??0;const pct=(p+100)/200;const left=p<0?pct*100:50;const width=Math.abs(p)/2;return<div style={{position:"absolute",left:`${left}%`,width:`${width}%`,height:"100%",borderRadius:2,background:"#8E8E93"}}/>;})()}
                      </div>
                      {/* thumb */}
                      <div style={{position:"absolute",left:`${((f.pan??0)+100)/200*100}%`,transform:"translateX(-50%)",width:14,height:14,borderRadius:"50%",background:"#8E8E93",boxShadow:"0 0 5px #8E8E9399",border:"2px solid rgba(0,0,0,0.5)",pointerEvents:"none"}}/>
                    </div>
          ...
          <div style={{padding:"16px 20px",display:"flex",flexDirection:"column",gap:18}}>
            {([
              {title:"Header",color:"#FF9500",icon:"🎸",items:[
                {key:"K (logo)",desc:"Icon that pulses on every downbeat — confirms audio is active."},
                {key:"Kit Browser",desc:"Browse and switch between kits: 808, CR-78 Vintage, factory kits and your saved custom kits. Click BROWSE to open."},
                {key:"Mascot",desc:"Animated drummer that hits in time with active tracks. Speed and halo glow follow the BPM."},
                {key:"↺ / ↻",desc:"Undo (Ctrl+Z) / Redo (Ctrl+Y) — up to 50 history steps on patterns and FX."},
                {key:"⊙ (Welcome)",desc:"Re-display the welcome/intro overlay. Useful if you want to show the app to someone new."},
                {key:"📖 (Manual)",desc:"Opens this user guide. Click anywhere outside to close."},
                {key:"? (Hints)",desc:"Desktop: hover any control to see its description tooltip. Mobile: activates Inspect Mode — a transparent overlay appears, tap any element to read its info without triggering it. Tap ✕ EXIT in the yellow banner to return to normal."},
                {key:"Theme",desc:"Toggle between dark and light theme. Purely visual, no effect on audio."},
              ]},
              {title:"Transport",color:"#30D158",icon:"▶",items:[
                {key:"▶ / ■ (Space)",desc:"Start or stop playback. The green button pulses on every beat when playing."},
                {key:"BPM — ‹ › or drag",desc:"Tempo in beats per minute (20–280). Use arrows, drag the slider, or use ← → keyboard shortcuts."},
                {key:"TAP",desc:"Tap several times in rhythm to detect the BPM automatically (TAP TEMPO)."},
                {key:"VOL",desc:"Master volume — drag vertically on the bar to adjust."},
                {key:"SWING",desc:"Shifts off-beats to create a shuffled groove. 0 = straight, ~67% = jazz triplet, 100% = maximum shuffle."},
                {key:"4/4 (time sig)",desc:"Choose the time signature: 4/4, 3/4, 6/8, 5/4, 7/8… Groups steps into bars and sets metronome accents."},
                {key:"METRO + sub",desc:"Audio metronome on/off. Drag vertically to adjust its volume. SUB cycles through off / eighth-note / sixteenth-note subdivisions."},
              ...
              {title:"Tracks — Sequencer",color:"#FF2D55",icon:"◆",items:[
                ...
                {key:"Step — drag ↕",desc:"Adjust step velocity (hit volume). Up = louder, down = softer. The step brightens with intensity."},
                {key:"Step — drag ↔",desc:"Nudge: shift the hit slightly early or late for an independent micro-swing per step."},
                {key:"Step — long-press",desc:"Open the probability popup (0–100%). The sound fires randomly at that percentage each cycle."},
                {key:"Step — double-tap",desc:"Reset step to default values (velocity 100%, nudge 0, probability 100%)."},
                {key:"M (Mute)",desc:"Silently mute the track without erasing its steps. Useful for live drop-outs."},
                {key:"S (Solo)",desc:"Isolate the track: all other tracks are muted. Tap again to return."},
                {key:"CLR",desc:"Clear all steps on this track only."},
                {key:"🎲 (Random vel)",desc:"Randomize the velocity of all active steps on the track. Undoable."},
                {key:"16st / 32st",desc:"Per-track step count. Each track can have a different length — enables polymetric patterns."},
                {key:"VOL / PAN",desc:"Track volume and stereo pan. Drag vertically. Double-click to reset to center/80%."},
                {key:"FX (per track)",desc:"Open per-track effects: pitch, filter, drive, reverb send, delay send, transient shaper."},
                {key:"Sample loader",desc:"Load an audio file (WAV, MP3, OGG) or record from the microphone as the track's sound source."},
                {key:"+ ADD TRACK",desc:"Create an extra custom track. Load any sample into it."},
              ]},
              {title:"Perform FX",color:"#5E5CE6",icon:"🎛",items:[
                {key:"PERFORM FX panel",desc:"Toggle with the 🎛 icon in the sequencer/Euclidean view. Real-time performance tools applied while held or dragged."},
                {key:"FILTER SWEEP",desc:"XY pad: drag left/right = cutoff frequency, up/down = resonance. Applied to the selected track or master bus."},
                {key:"STUTTER",desc:"Hold = rhythmic repeat at the selected subdivision (1/4, 1/8, 1/16, 1/32). On master: mute/unmute flutter. On a track: pad repeat."},
                {key:"REV HOLD",desc:"Hold to instantly boost the reverb on all active tracks. Release = smooth return to the original level."},
                {key:"DLY HOLD",desc:"Hold to activate a boosted delay at the selected subdivision. Release = fades back to the previous delay level."},
                {key:"Division (1/4…1/32)",desc:"Shared subdivision selector for both STUTTER and DLY HOLD timing."},
              ]},
              {title:"FX Rack global",color:"#BF5AF2",icon:"⚙",items:[
                {key:"PRESETS",desc:"Load a genre preset (Trap, Boom Bap, Techno, Lo-Fi, Afrobeat…) that configures all global FX at once."},
                {key:"BYPASS",desc:"Disable all global effects in one click — useful for comparing dry vs. processed sound."},
                {key:"CHAIN order",desc:"Reorder the master bus series chain (Drive → Comp → Filter) by drag or arrows. Order affects the character of the sound."},
                {key:"PRE / POST",desc:"Choose whether send FX (reverb, delay…) receive the signal before or after the series chain."},
                {key:"REVERB",desc:"Algorithmic reverb: Plate, Room or Hall. Decay = tail length, Size = room size. Reverb send is also per-track in the FX panel."},
                {key:"DELAY",desc:"Stereo echo. BPM-synced or free time. Feedback = number of repeats. Delay send is per-track too."},
                {key:"FILTER",desc:"Master low-pass / high-pass / band-pass filter. Optional LFO (sine / triangle / square) for auto-wah or rhythmic sweeps."},
                {key:"COMP + GR",desc:"Master compressor. The GR meter shows gain reduction in real time."},
                {key:"DRIVE",desc:"Master saturation — four modes: tube, tape, triangle, bit-crush."},
                {key:"TRANSIENT",desc:"Per-track attack/sustain shaper: boost or cut the attack punch and body independently."},
              ]},
              {title:"Pattern Bank",color:"#64D2FF",icon:"◧",items:[
                {key:"PAT 1–8",desc:"Pattern slots. Tap to switch the active pattern. Long-press a slot to rename it."},
                {key:"16 / 32 steps",desc:"Change the step count for the current pattern (applies to all tracks)."},
                {key:"TEMPLATES",desc:"Load a preset groove: 808 Classic, Trap, Jazz Brushes, Afrobeat, Reggaeton, etc. Inserted into the current slot without touching other patterns."},
                {key:"DUP",desc:"Duplicate the current pattern into the next slot."},
                {key:"CLR",desc:"Clear all hits in the current pattern (keeps the slot)."},
                {key:"DEL",desc:"Delete the current pattern slot entirely."},
              ]},
              {title:"Song Arranger",color:"#FF9500",icon:"♪",items:[
                {key:"SONG ON / OFF",desc:"Enable Song Mode to chain patterns in row order. The sequencer follows the arrangement instead of looping one pattern."},
                {key:"Slot grid",desc:"Each row holds up to 16 slots. Tap a slot to assign a pattern. Tap again to clear it."},
                {key:"Dup row / Del row",desc:"Duplicate a row right below it, or delete an entire row of slots."},
                {key:"+ New row",desc:"Add an empty row at the bottom of the arrangement."},
                {key:"Clear all",desc:"Empty every slot in every row (patterns themselves are kept)."},
              ]},
              {title:"Euclidean Sequencer",color:"#FFD60A",icon:"⬡",items:[
                {key:"N (cycle length)",desc:"Total number of subdivisions in the Euclidean cycle — the denominator of the rhythm."},
                {key:"HITS",desc:"Number of sounds placed mathematically across N steps by the Euclidean algorithm."},
                {key:"ROT (rotation)",desc:"Shifts the start point of the pattern — changes which beat is the 'one'."},
                {key:"Drag ↕ on values",desc:"All N / HITS / ROT values can be adjusted by dragging vertically on the number."},
                {key:"Polyrhythm presets",desc:"12 presets from world music traditions: Clave, Bembé, Tresillo, Habanera, Bossa Nova, West African…"},
                {key:"EDIT view",desc:"Expand a track to see the generated pattern as a step grid and manually add or remove hits."},
              ]},
              {title:"Live Pads",color:"#5E5CE6",icon:"⊞",items:[
                {key:"Colored pads",desc:"Play each track in real time. Works with touch, mouse click, or keyboard key assignments."},
                {key:"Velocity",desc:"Hold a pad slightly longer before releasing for a louder hit — simulates velocity sensitivity."},
                {key:"REC + pads",desc:"Enable REC in the transport, then hit pads to record live hits directly into the sequencer steps."},
                {key:"Keyboard keys",desc:"Assign keys to tracks via the KEYB button in the transport bar. Assigned keys play the matching pad."},
              ]},
            ...
            <div style={{textAlign:"center",fontSize:7,color:th.faint,paddingTop:6,borderTop:"1px solid rgba(255,255,255,0.06)"}}>Click outside to close · Kick & Snare v{APP_VERSION} — DRUM EXPERIENCE · Mobile: use ? to inspect any control without triggering it</div>
          </div>
        </div>
      </div>
    )}
        ...
        <div onClick={e=>e.stopPropagation()} style={{position:"fixed",bottom:60,left:"50%",transform:"translateX(-50%)",width:"min(440px,96vw)",borderRadius:14,background:th.surface,border:`1px solid ${th.sBorder}`,padding:"20px 18px",boxShadow:"0 8px 40px rgba(0,0,0,0.7)"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"6px 18px"}}>
            {[
              ...
              ["Double-tap step","Reset step"],
              ["Long-press PAT","Rename pattern"],
              ["EUCLIDEAN → EDIT","Visualise & edit hits"],
              ["? (mobile)","Inspect Mode — tap to inspect"],
            ].map(([k,v])=>(
